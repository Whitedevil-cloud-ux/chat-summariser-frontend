...
function Dashboard() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this conversation?")) {
      await fetch(`${API_BASE}/chat/${id}/delete/`, { method: "DELETE" });
      fetchConversations();
    }
  };
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="relative mb-6 md:mb-8">
          <input
            ...
            className="w-full px-4 py-2.5 md:py-3 pl-10 rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:outline-none focus:ring focus:ring-indigo-400 shadow-sm"
          />
          <Search className="absolute left-3 top-2.5 md:top-3 text-gray-400 dark:text-gray-500" size={18} />
        </div>
        ...
        {filtered.length ? (
          ...
          >
            {filtered.map((conv) => (
              <motion.div
                key={conv.id}
                className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-100 dark:border-gray-700 p-4 md:p-5 flex flex-col justify-between hover:shadow-xl transition"
